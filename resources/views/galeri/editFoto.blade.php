                    ...
                    <form action="{{ route('foto.update', $foto->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <input value="{{ old('judul', $foto->judul) }}" name="judul"
                                    placeholder="Masukkan Judul Foto" type="text"
                            ...
                            <div>
                                ...
                                <input id="tags" name="tags" placeholder="Tags" type="text"
                                    ...
                                    value="{{ old('tags', $foto->tags) }}">
                            </div>
                            <div>
                                <label for="tanggal_unggah" class="text-lg font-medium">Tanggal Publish</label>
                                <input value="{{ old('tanggal_unggah', $foto->tanggal_unggah) }}" name="tanggal_unggah"
                            ...
                            <div>
                                ...
                                <select id="status_foto" name="status_foto"
                                    ...
                                    <option value="validasi" {{ $foto->status_foto == 'validasi' ? 'selected' : '' }}>
                                        Perlu Validasi</option>
                                    <option value="publish" {{ $foto->status_foto == 'publish' ? 'selected' : '' }}>
                                        Publish</option>
                                    <option value="draft" {{ $foto->status_foto == 'draft' ? 'selected' : '' }}>Draft
                                    </option>
                                </select>
                            </div>
                            <div>
                                <label for="tanggal_publish_mulai" class="text-lg font-medium">Tanggal Mulai
                                    Publish</label>
                                <input value="{{ old('tanggal_publish_mulai', $foto->tanggal_publish_mulai) }}"
                            ...
                            <div>
                                <label for="tanggal_publis_selesai" class="text-lg font-medium">Tanggal Selesai
                                    Publish</label>
                                <input value="{{ old('tanggal_publis_selesai', $foto->tanggal_publis_selesai) }}"
                            ...
                            <div class="col-span-2">
                                ...
                                <div id="editor" class="border-gray-300 shadow-sm w-full rounded-lg p-2">
                                    {!! old('deskripsi', $foto->deskripsi) !!}</div>
                                <input type="hidden" name="deskripsi" id="editor-content"
                                    value="{{ old('deskripsi', $foto->deskripsi) }}">
                            </div>
                            <div>
                            ...
                            <div id="preview" class="mt-4 flex flex-wrap gap-2">
                                @if ($foto->getMedia('images')->isNotEmpty())
                                    @foreach ($foto->getMedia('images') as $image)
