<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            @can('create apa siapa')
                <a href="{{ route('apa_siapa.create') }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2">Create</a>
            @endcan
        </div>
    </x-slot>
            ...
            <table class="w-full">
                <thead class="bg-gray-500 text-white">
                    <tr class="border-b">
                        ...
                        <th class="px-6 py-4 text-center">Tanggal Kegiatan</th>
                        <th class="px-6 py-4 text-center">Action</th>
                    </tr>
                </thead>
                <tbody class="bg-white">
                    @if ($apaSiapa->isNotEmpty())
                        @foreach ($apaSiapa as $item)
                            <tr class="border-b">
                                ...
                                <td class="px-6 py-4 text-center">
                                    {{ \Carbon\Carbon::parse($item->tanggal_kegiatan_mulai)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        @can('update apa siapa')
                                            <a href="{{ route('apa_siapa.edit', $item->id) }}"
                                                class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-600">
                                                Edit
                                            </a>
                                        @endcan
                                        @can('delete apa siapa')
    ...
    <x-slot name="script">
        <script type="text/javascript">
            ...
            document.getElementById("confirmDelete").addEventListener("click", function() {
                if (deleteId) {
                    fetch('{{ route('apa_siapa.destroy', ':id') }}'.replace(':id', deleteId), {
                            method: "DELETE",
                            headers: {
                        ...
                        .then(data => {
                            if (data.status) {
                                closeDeleteModal();
                                window.location.href = '{{ route('apa_siapa.list') }}'; // Redirect langsung
                            }
                        })
                        .catch(error => {
